...
async function loadLogoDataUrl(): Promise<string> {
  ...
  if (!response.ok) throw new Error('Unable to load the Point2Wealth logo');
...
export async function generateTradePdf(
  ...
  doc.addImage(logoDataUrl, 'PNG', margin, 8, 42, 17);
  ...
  autoTable(doc, {
    ...
    didDrawPage(data) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.text('Generated by Point2Wealth', margin, pageH - 8);
      doc.text(`Page ${pageNum} of ${totalPages}`, pageWidth - margin, pageH - 8, { align: 'right' });

      if (pageNum > 1) {
        doc.addImage(logoDataUrl, 'PNG', margin, 3, 25, 10);
      }
    },
  });
