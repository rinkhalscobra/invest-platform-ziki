/*
  # Fix close_prop_position function PnL calculation

  1. Function Updates
    - Remove leverage multiplication from PnL calculation in close_prop_position function
    - PnL should be: (price_difference * amount) without leverage multiplication
    - Leverage only affects margin requirements and ROI calculation, not raw PnL

  2. Changes Made
    - Updated long position PnL: (current_price - entry_price) * amount (removed * leverage)
    - Updated short position PnL: (entry_price - current_price) * amount (removed * leverage)
    - ROI calculation remains the same: (pnl / margin) * 100
*/
