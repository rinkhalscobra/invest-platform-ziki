/*
  # Fix Close Futures Position Balance Table Reference

  1. Changes
    - Update `close_futures_position` function to use `balances` table instead of `users` table
    - Fix referrer commission to update `balances.usdt_balance`
    - Fix user balance update to use `balances.usdt_balance`

  2. Description
    - The function was incorrectly trying to update `users.usdt_balance` which doesn't exist
    - User balances are stored in the `balances` table
    - This fixes the "column usdt_balance does not exist" error
*/

-- Drop and recreate close_futures_position function with correct table reference
...
BEGIN
  ...
  IF v_referrer_id IS NOT NULL AND v_net_pnl > 0 THEN
    ...
    -- Add commission to referrer's balance (FIX: use balances table)
    UPDATE balances
  ...
  -- Return margin + net PnL to user's balance (FIX: use balances table)
  UPDATE balances
