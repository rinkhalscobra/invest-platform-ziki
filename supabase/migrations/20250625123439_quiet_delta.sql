...
DO $$
BEGIN
  -- Check if "Users can manage own assets" policy exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_assets' AND policyname = 'Users can manage own assets'
  ) THEN
  ...
  -- Check if "Service role can manage all assets" policy exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_assets' AND policyname = 'Service role can manage all assets'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_user_assets_updated_at'
    AND tgrelid = 'user_assets'::regclass
  ) THEN
