/*
  # Fix Close Position to Only Return PnL (Not Margin) - v2

  1. Changes
    - Update `close_futures_position` function to ONLY return net PnL to user's balance
    - Remove margin from the return calculation
    - Margin is NOT deducted when opening (frontend inserts directly), so we should NOT return it when closing

  2. Description
    - Currently, positions are opened by direct INSERT in frontend without deducting margin
    - When closing, we were incorrectly returning margin + PnL
    - This caused users to get free money (margin they never paid)
    - Now we only return the net PnL (profit or loss after all costs)
*/

-- Drop and recreate close_futures_position function to only return PnL
