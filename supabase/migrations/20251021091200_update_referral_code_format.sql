/*
  # Update Referral Code Format - Part 1: Function Creation

  1. Changes
    - Create function to generate short referral codes (e.g., S23DHDS)
    - Modify user creation trigger to use new format

  2. Format
    - 7 characters: 1 letter + 5 alphanumeric + 1 letter
    - Example: S23DHDS, A7X2K9P, M4N8T3R

  3. Note
    - Data migration to update existing codes is in next migration file
*/
...
-- Update the handle_email_verified_user function to use new referral code format
...
BEGIN
  ...
  IF NEW.email_confirmed_at IS NOT NULL AND OLD.email_confirmed_at IS NULL THEN
    -- Insert into public.users with new referral code format
    INSERT INTO public.users (id, email, kyc_status, referral_code, is_demo)
    VALUES (NEW.id, NEW.email, 'not_verified', generate_referral_code(), false)
